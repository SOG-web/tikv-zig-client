...
pub const Encoder = struct {
    ...
    fn encodeHeaderField(self: *Encoder, buffer: *std.ArrayList(u8), name: []const u8, value: []const u8) !void {
        ...
        try buffer.append(0x40); // Pattern: 01
    ...
    fn encodeInteger(self: *Encoder, buffer: *std.ArrayList(u8), value: u32, prefix_bits: u3, pattern: u8) !void {
        ...
        if (value < max_prefix) {
            try buffer.append(pattern | @as(u8, @intCast(value)));
        } else {
            try buffer.append(pattern | @as(u8, @intCast(max_prefix)));
            var remaining = value - max_prefix;
            while (remaining >= 128) {
                try buffer.append(@as(u8, @intCast((remaining % 128) + 128)));
                remaining /= 128;
            }
            try buffer.append(@as(u8, @intCast(remaining)));
        }
    }
