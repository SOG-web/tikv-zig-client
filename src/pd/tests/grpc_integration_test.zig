...
test "PD gRPC client endpoint parsing" {
    ...
    // Test endpoint parsing (this will fail without a real PD server, but tests the parsing logic)
    const result = client.ptr.getGrpcClient();

    // Should fail with RpcError since no real server is running
...
test "PD gRPC region operations fallback" {
    ...
    // Test GetRegion with gRPC enabled (should fallback to HTTP, which will also fail but tests the path)
    const region_result = client.getRegion(test_key, false);

    // Should fail with RpcError since no real server is running
    try expectError(types.Error.RpcError, region_result);
}
...
test "PD gRPC store operations fallback" {
    ...
    // Test GetStore with gRPC enabled (should fallback to HTTP)
    const store_result = client.getStore(1);

    // Should fail with RpcError since no real server is running
    try expectError(types.Error.RpcError, store_result);

    // Test GetAllStores with gRPC enabled (should fallback to HTTP)
    const stores_result = client.getAllStores();

    // Should fail with RpcError since no real server is running
    try expectError(types.Error.RpcError, stores_result);
}
...
test "PD gRPC client memory management" {
    ...
    for (0..10) |_| {
        ...
        try expectEqual(@as(usize, 3), client.ptr.endpoints.len);
